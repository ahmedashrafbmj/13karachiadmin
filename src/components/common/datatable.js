...
const Datatable = ({ myData, myClass, multiSelectOption, pagination }) => {
	...
	const approve = (rowId, rejectorApp) => {
	...
	const handleEditFormSubmit = async () => {
		...
		formData.append('price', inpdata.price);
		formData.append('discountedPrice', inpdata.discountedPrice);
		formData.append('menuProductNumber', inpdata.menuProductNumber);
		formData.append('shortDescription', inpdata.shortDescription);
		formData.append('totalUnits', inpdata.totalUnits);
		formData.append('ProductLink', inpdata.ProductLink);
		formData.append('longDescription', inpdata.longDescription);
		...
		try {
			...
			const response = await axios.put(`${baseurl.url}updateProduct/${editingRow._id}`, formData, {
				headers: {
	...
	const handleCheckboxChange = (e, type) => {
		...
		if (isChecked) {
			...
			if (type === 'selectedSubcategory') {
				setSelectedSubcategories(selectedSubcategories.filter((category) => category._id !== brandId));
			}
		}
	};
		...
		},
		{
			...
			cell: (row) => (
				<div className="d-flex col-12">
					<button onClick={() => editRow(row)} className="btn btn-secondary ms-3 shadow btn-xs sharp me-1">
					...
					<Fragment>
						...
						<Container fluid={true}>
											...
											<div className="digital-add needs-validation">
												<FormGroup>
													...
													<Input
														...
														name="name"
														defaultValue={editingRow ? editingRow.name : inpdata.name}
													/>
												</FormGroup>
												<FormGroup>
													<Label className="col-form-label pt-0">
													...
													<Input
														...
														defaultValue={editingRow ? editingRow.price : inpdata.price}
													/>
												</FormGroup>
												<FormGroup>
													<Label className="col-form-label pt-0">
                            ...
                            />
												...
												{cat?.map((e, i) => {
													const isChecked = editingRow?.category?.some(
                        ...
                        <option defaultValue="5">Websites</option>
												...
												<FormGroup>
													...
													<Input
														...
														defaultValue={
															editingRow ? editingRow.ProductLink : inpdata.ProductLink
														}
													/>
												</FormGroup>
												<FormGroup>
