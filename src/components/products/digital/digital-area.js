...
const Digital_area = () => {
	...
	const handleFormSubmit = async () => {
		...
		if (editingRow?._id) {
			try {
				...
				const response = await axios.put(`${baseurl.url}updateArea/${editingRow?._id}`, formData, {
					headers: {
			...
			try {
				...
				const response = await axios.post(`${baseurl.url}addArea`, formData, {
					headers: {
						'Content-Type': 'multipart/form-data', // Important for sending form data
					},
				});
	...
	const fetchDataFromServer = async () => {
		try {
			...
			setRes(response.data.areas);
			setimage(response.data.areas.map((e, i) => `${baseurl.image + e.image[0]}`));
