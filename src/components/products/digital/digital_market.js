...
const Digital_market = () => {
	...
	const handleFormSubmit = async () => {
		...
		if (editingRow?._id) {
			try {
				...
				const response = await axios.put(`${baseurl.url}updateMarket/${editingRow?._id}`, formData, {
					headers: {
			...
			try {
				...
				const response = await axios.post(`${baseurl.url}addMarket`, formData, {
					headers: {
						'Content-Type': 'multipart/form-data', // Important for sending form data
	...
	const fetchDataFromServer = async () => {
		try {
			...
			setimage(response.data.market.map((e, i) => `${baseurl.image + e.image[0]}`));
					...
					<Col sm="12">
                      ...
                      <span>*</span>Select Color 
													...
													<Input
														...
														defaultValue={editingRow?._id ? editingRow?.area : null}
