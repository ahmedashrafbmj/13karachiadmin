...
const Digital_sub_category = () => {
		...
		},
		{
			...
			cell: (row) => (
				<div className="text-center">
					<button onClick={() => editRow(row)} className="btn btn-success p-2 shadow btn-xs sharp me-1">
						<div
							style={{
								display: 'flex',
								alignItems: 'center',
	...
	const handleFormSubmit = async () => {
		...
		if (editingRow?._id) {
			try {
				...
				const response = await axios.put(`${baseurl.url}updatesubCategories/${editingRow?._id}`, formData, {
					headers: {
			...
			try {
				...
				const response = await axios.post(`${baseurl.url}addsubCategory`, formData, {
					headers: {
						'Content-Type': 'multipart/form-data', // Important for sending form data
					},
				});
				...
				if (response.data.status) {
					...
					const updatedData = res.filter((item) => item._id !== editingRow?._id);
					setRes(updatedData);
				}
				window.location.reload();
				setOpen(false);
	...
	const fetchDataFromServer = async () => {
		try {
			...
			setimage(response.data.brands.map((e, i) => `${baseurl.image + e.image[0]}`));
			handleFormSubmit();
			fetchDataFromServer();
	...
	const handleCheckboxChange = (e, type) => {
		...
		if (isChecked) {
			...
			setSelectedCategory(SelectedCategory.filter((brand) => brand._id !== brandId));
						...
						<Card>
							...
							<CardBody>
								<div className="btn-popup pull-right">
									{localStorage.getItem('role') === 'subadmin' ? null : (
										<Button
											type="button"
											color="secondary"
											onClick={onOpenModal}
											...
											<Form>
												<FormGroup>
													...
													<Input
														...
														defaultValue={editingRow?._id ? editingRow?.name : null}
													/>
												</FormGroup>
												<p>Select Categories</p>
												{cat?.map((e, i) => {
													const isChecked = editingRow?.categories?.includes(e?.name); // Invert the value
														...
														<FormGroup check key={i}>
															<Label check>
																<Input
																	...
																	onChange={(event) =>
																		handleCheckboxChange(event, 'selectedCategory')
																	}
																/>{' '}
																{e?.name}
															</Label>
								...
								<div id="basicScenario" className="product-physical">
									<DataTable
										data={res}
										columns={
											localStorage.getItem('role') === 'subadmin' ? columnsSubAdmin : columnsAdmin
										}
										pagination={false}
										striped={true}
										center={true}
